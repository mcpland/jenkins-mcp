...
export async function getAllQueueItems(runtime: ToolRuntime): Promise<Array<Record<string, unknown>>> {
...
export async function getQueueItem(runtime: ToolRuntime, id: number): Promise<Record<string, unknown>> {
  const jenkins = await runtime.getJenkins();
